...
uint32_t CLOCK_GetCmpFClkFreq(uint32_t id)
{
    ...
    switch ((id == 0U) ? (SYSCON->CMP0FCLKSEL) : (SYSCON->CMP1FCLKSEL))
    {
        case 1U:
            freq = CLOCK_GetPll0OutFreq();
            break;
    ...
    div = ((id == 0U) ? ((SYSCON->CMP0FCLKDIV & SYSCON_CMP0FCLKDIV_DIV_MASK) + 1U) :
                        ((SYSCON->CMP1FCLKDIV & SYSCON_CMP1FCLKDIV_DIV_MASK) + 1U));
...
uint32_t CLOCK_GetCmpRRClkFreq(uint32_t id)
{
    ...
    switch ((id == 0U) ? (SYSCON->CMP0RRCLKSEL) : (SYSCON->CMP1RRCLKSEL))
    {
        case 1U:
            freq = CLOCK_GetPll0OutFreq();
            break;
...
/* Get UTICK Clk */
/*! brief  Return Frequency of UTICK
 *  return Frequency of UTICK
 */
uint32_t CLOCK_GetUtickClkFreq(void)
{
    uint32_t freq = 0U;
    uint32_t div  = ((SYSCON->UTICKCLKDIV & SYSCON_UTICKCLKDIV_DIV_MASK) + 1U);

    switch (SYSCON->UTICKCLKSEL)
    {
        case 0U:
            freq = CLOCK_GetExtClkFreq();
            break;
